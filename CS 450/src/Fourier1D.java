...
public class Fourier1D {
    ...
    public Complex2[] fft (Complex2[] v) {

        Complex2[] vf;
        Complex2 omg, tmp;
        int m;

        vf = new Complex2[N_2P];
        for(int i =0; i < N_2P; i++) {
            vf[bitrev(i)] = new Complex2(v[i]);
        }
        ...
        for (int k = 0; k <= maxPot; k++) {
            ...
            j = 2*m;
            for (int L = 0; L < m; L++) {
                omg = Complex2.expi(-L*Math.PI/m);
                for (int M = 0; M <= N_2P-j; M+=j) {
                    tmp = Complex2.mult(omg,vf[L+M+m]);
                    vf[L+M+m] = Complex2.sub(vf[L+M],tmp);
                    vf[L+M] = Complex2.add(vf[L+M],tmp);
                }
            }
        }
    ...
    public Complex2[] ifft (Complex2[] v) {
