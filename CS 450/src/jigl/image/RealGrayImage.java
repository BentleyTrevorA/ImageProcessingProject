...
public class RealGrayImage implements Image {
	 ...
	 * @param data one dimensional array of float. The array is length x*y.
	 */
	...
	*/
	...
	public void byteSize() {
		...
		for (int y = 0; y < Y; y++) {
			for (int x = 0; x < X; x++) {
				value = ((255.0f / range) * ((float) data[y][x] - min));
				//value = 0x00FF & value;
				data[y][x] = value;
			}
		}
	...
	public final void set(int x, int y, float value, ROI roi) {
		data[(roi.uy() + y)][(roi.ux() + x)] = (float) value;
	}
	 ...
	 * @return this
	 */
	public final RealGrayImage add(float v, ROI roi) {
		float sv = (float) v;
		for (int y = roi.uy(); y <= roi.ly(); y++) {
			for (int x = roi.ux(); x <= roi.lx(); x++) {
				data[y][x] += sv;
	 ...
	 * @return this
	 */
	public final RealGrayImage subtract(float v, ROI roi) {
		float sv = (float) v;
		for (int y = roi.uy(); y <= roi.ly(); y++) {
			for (int x = roi.ux(); x <= roi.lx(); x++) {
				data[y][x] -= sv;
	 ...
	 * @return this
	 */
	public final RealGrayImage multiply(float v, ROI roi) {
		float sv = (float) v;
		for (int y = roi.uy(); y <= roi.ly(); y++) {
			for (int x = roi.ux(); x <= roi.lx(); x++) {
				data[y][x] *= sv;
	 ...
	 * @return this
	 */
	public final RealGrayImage divide(float v, ROI roi) {
		float sv = (float) v;
		for (int y = roi.uy(); y <= roi.ly(); y++) {
			for (int x = roi.ux(); x <= roi.lx(); x++) {
				data[y][x] /= sv;
