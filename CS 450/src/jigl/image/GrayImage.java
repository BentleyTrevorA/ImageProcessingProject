...
public class GrayImage implements Image {
	 ...
	 * @param data one dimensional array of short. The array is length x*y.
	 */

	public GrayImage(int x, int y, short[] data) {
		X = x;
		Y = y;
		this.data = new short[Y][X];
		for (int a = 0; a < y; a++) {
	 ...
	 * 
	 * @param data two dimensional array of short
	 */
	...
	*/
	 ...
	 * @param roi Region of Interest
	 */
	public final void set(int x, int y, int value, ROI roi) {
	 ...
	 * @return this
	 */
	public final GrayImage add(GrayImage im, ROI sourceROI, ROI destROI) {
	 ...
	 * @return this
	 */
	public final GrayImage subtract(GrayImage im, ROI sourceROI, ROI destROI) {
		for (int y = destROI.uy(); y <= destROI.ly(); y++) {
			for (int x = destROI.ux(); x <= destROI.lx(); x++) {
				data[x][y] -= im.get((x - destROI.ux() + sourceROI.ux()),
						(y - destROI.uy() + sourceROI.uy()));
	 ...
	 * @return this
	 */
	public final GrayImage multiply(GrayImage im, ROI sourceROI, ROI destROI) {
		for (int y = destROI.uy(); y <= destROI.ly(); y++) {
			for (int x = destROI.ux(); x <= destROI.lx(); x++) {
				data[x][y] *= im.get((x - destROI.ux() + sourceROI.ux()),
						(y - destROI.uy() + sourceROI.uy()));
	 ...
	 * @return this
	 */
	public final GrayImage divide(GrayImage im, ROI sourceROI, ROI destROI) {
		for (int y = destROI.uy(); y <= destROI.ly(); y++) {
			for (int x = destROI.ux(); x <= destROI.lx(); x++) {
				data[x][y] /= im.get((x - destROI.ux() + sourceROI.ux()),
						(y - destROI.uy() + sourceROI.uy()));
