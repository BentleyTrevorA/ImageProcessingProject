...
public class LocalDifferentialGeometry {
	...
	public RealGrayImage calcIxx(int x1, int y1, int x2, int y2) {
		...
		try {
			ImageKernel ik = new ImageKernel(9); // D_XX
			Convolve c = new Convolve(ik);
			IxxImage = (RealGrayImage) c.apply((Image) image, new ROI(x1, y1, x2, y2));
	...
	public RealGrayImage calcIxy(int x1, int y1, int x2, int y2) {
		...
		try {
			ImageKernel ik;
			ik = new ImageKernel(11); // D_XY

			Convolve c = new Convolve(ik);
			IxyImage = (RealGrayImage) c.apply((Image) image, new ROI(x1, y1, x2, y2));
