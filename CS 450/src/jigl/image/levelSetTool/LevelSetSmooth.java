...
public class LevelSetSmooth {
	 ...
	 */
	void setAutoParameters(RealColorImage image, double mean, double std, double std2) {
		int X = image.X();
		int Y = image.Y();
		double val = 0.0;
		double val2 = 0.0;
	 ...
	 * 
	 * @param img - a pointer to the original image
	 * @param mean - the mean of delta (the average difference between a pixel and its
	 *            eight-connected neighbors)
			...
			for (int x = x1; x < x2; x++) {
				for (int y = y1; y < y2; y++) {
					if (want.get(x - x1, y - y1) > 255)
						want.set(x - x1, y - y1, 255);
					else if (want.get(x - x1, y - y1) < 0)
						want.set(x - x1, y - y1, 0);
