...
public class SequenceCanvas extends Canvas implements Runnable, ActionListener {
	...
	public SequenceCanvas(ImageSequence sequence) throws BadImageException {
		...
		for (int a = 0; a < sequence.number(); a++)
			images.addElement(new ImageCanvas(sequence.get(a)));
	...
	public void add(jigl.image.Image image) throws BadImageException {
		images.addElement(new ImageCanvas(image));
		if (images.size() == 1) {
	...
	public ImageCanvas getCanvas(int index) {
		return (ImageCanvas) images.elementAt(index);
	}
	...
	public void paint(Graphics g) {
		ImageCanvas canvas = (ImageCanvas) images.elementAt(displayed);
		canvas.paint(g);
	...
	public void next() throws BadImageException {
		if (displayed < images.size() - 1)
			...
			displayed = 0;
		ImageCanvas tcanvas = (ImageCanvas) images.elementAt(displayed);
	...
	/** Starts the animation. Note: this method calls run atomatically */
	public void start() {
		if (frozen) {
