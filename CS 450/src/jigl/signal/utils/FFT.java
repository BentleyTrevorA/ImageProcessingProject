...
public class FFT {
	 ...
	 */
	static public ComplexSignal doFFT(Signal im, boolean forward) {
		int x = 0;
		int lgn = 0;
		int n = 0;
		int rev = 0;
		...
		if (!isPow2) {
			System.out
					.println("Error: The length of this signal is not a power of 2. You can pad zeros to change its length and apply FFT again.");
			return null;
		}
					...
					try {
						if (forward == true) {
							if (im instanceof DiscreteSignal) {
								...
								if (y < ci.length()) {
									a[0][rev] = (float) ci.getReal(y);
									a[1][rev] = (float) ci.getImag(y);
								} else {
									a[0][rev] = 0f;
									a[1][rev] = 0f;
								}
